...
namespace SpeechSample
{
    ...
    public partial class Window1 : Window
    {
        ...
        private void OpenRtfFileButton_Click(object sender, RoutedEventArgs e)
        {
            OpenFileDialog dlg = new OpenFileDialog();
            dlg.InitialDirectory = "c:\\";
            dlg.Filter = "Text files (*.rtf)|*.rtf|All Files (*.*)|*.*";
            dlg.RestoreDirectory = true;
            if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void LoadRtfDocument(string fileName)
        {
            TextRange range;
            System.IO.FileStream fStream = null;
            if (System.IO.File.Exists(fileName))
